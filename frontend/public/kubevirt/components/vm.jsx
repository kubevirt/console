...
import { CreateVmWizard, templates } from 'kubevirt-web-ui-components/dist/js';
...
export const VirtualMachinesPage = connect(
  ...
  openNewVmWizard = () => {
    const resources = [{ kind:NamespaceModel.kind, isList: true, prop: NamespaceModel.kind}];
    return <Firehose resources={resources} flatten={getFlattenForKind(NamespaceModel.kind)}>
      <ConnectedNewVMWizard onHide={this.onHide} createVm={k8sCreate} />
    </Firehose>;
  }

  render() {
    return <React.Fragment>
      {this.state.showWizard ? this.openNewVmWizard() : undefined}
      <ListPage
        {...this.props}
        canCreate={true}
