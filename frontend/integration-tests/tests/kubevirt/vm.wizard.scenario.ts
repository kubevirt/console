...
describe('Kubevirt create VM using wizard', () => {
  ...
  it('Configures VM Basic Settings', async() => {
    ...
    await vmView.nameInput.sendKeys(testName);
  ...
  it('Confirms to create VM', async() => {
    leakedResources.add(JSON.stringify({name: testName, namespace: testName, kind: 'vm'}));
  ...
  it('Verifies created VM', async() => {
    ...
    await filterForName(testName);
    await resourceRowsPresent();
    await browser.wait(until.textToBePresentInElement(vmView.firstRowVMStatus, 'Running'), 20000);
  });

  it('Removes created VM', async() => {
    await deleteRow('VirtualMachine')(testName);
    leakedResources.delete(JSON.stringify({name: testName, namespace: testName, kind: 'vm'}));
  });
});
